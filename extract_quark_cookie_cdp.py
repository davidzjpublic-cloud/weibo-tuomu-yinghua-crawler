...
chrome_quark_profile）。首次运行时，请在弹出的 Chrome 窗口中登录夸克网盘；
登录成功后脚本会自动提取 Cookie 并写入 config.json 的 quark_cookie 字段。
若该配置文件中仍有有效登录态，后续运行可直接提取，无需再次登录。

使用方法：
    python extract_quark_cookie_cdp.py

首次使用需交互：在弹出的 Chrome 窗口中完成夸克网盘登录。
"""
...
QUARK_PROFILE_DIR = Path(__file__).resolve().parent / "chrome_quark_profile"
REMOTE_DEBUGGING_PORT = 9222
QUARK_URL = "https://pan.quark.cn"
QUARK_API_HOST = "https://drive-m.quark.cn"
...
def launch_chrome(log_path: Path = None) -> subprocess.Popen:
    """启动带远程调试端口的 Chrome，使用独立的夸克爬虫配置目录。"""
    chrome = find_chrome_exe()
    if not chrome:
        print("未找到 chrome.exe，请确认 Google Chrome 已安装。")
        return None

    QUARK_PROFILE_DIR.mkdir(parents=True, exist_ok=True)

    cmd = [
        chrome,
        f"--remote-debugging-port={REMOTE_DEBUGGING_PORT}",
        "--remote-allow-origins=*",
        f"--user-data-dir={QUARK_PROFILE_DIR}",
    ...
    print(f"独立配置目录: {QUARK_PROFILE_DIR}")
    if log_path:
...
def get_page_ws_url() -> str:
    ...
    for page in pages:
        if page.get("type") == "page" and "quark" in (page.get("url") or ""):
            return page.get("webSocketDebuggerUrl")
    for page in pages:
        if page.get("type") == "page":
            return page.get("webSocketDebuggerUrl")

    new_url = f"http://127.0.0.1:{REMOTE_DEBUGGING_PORT}/json/new?{QUARK_URL}"
...
def wait_for_login(ws_url: str, timeout: int = 300) -> list:
    ...
    while time.time() < deadline:
        ...
        # 夸克转存 API 通常需要 __pus / __puus / __kp 同时存在
...
def write_config(cookie_str: str) -> None:
    ...
    old_cookie = config.get("quark_cookie", "")
...
def main():
    print("=" * 50)
    print("夸克网盘 Cookie 提取工具 (CDP 版)")
    print("=" * 50)

    log_path = Path(__file__).resolve().parent / "chrome_cdp_debug.log"
    ...
    try:
        ...
        quark_cookies = wait_for_login(ws_url, timeout=300)
        if not quark_cookies:
            print("\n未获取到可用于转存的夸克登录 Cookie，config.json 未被修改。")
            sys.exit(1)

        # 登录完成，隐藏窗口减少干扰
        hide_chrome_window(proc.pid)

        cookie_str = build_cookie_string(quark_cookies)
        print(f"\n获取到 {len(quark_cookies)} 个夸克 Cookie: {', '.join(c['name'] for c in quark_cookies)}")
